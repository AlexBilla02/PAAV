...
class Simulation:
    ...
    def kinematic_model(self, ax, delta):
        ...
        x, y, theta, vx, vy, r = self.x, self.y, self.theta, self.vx, self.vy, self.r
    ...
    def linear_single_track_model(self, ax, delta):
        ...
        x, y, theta, vx, vy, r = self.x, self.y, self.theta, self.vx, self.vy, self.r
        Cf = self.Cf  # Stiffness anteriore
        Cr = self.Cr  # Stiffness posteriore
        lf = self.l_f # Distanza asse anteriore
        lr = self.l_r # Distanza asse posteriore
        m = self.mass # Massa del veicolo
        Iz = self.I_z # Momento d'inerzia
        # Tire slip angles
        alpha_f = delta - (vy + lf * r) / vx if vx != 0 else 0  # Angolo di scivolamento anteriore
        alpha_r = - (vy - lr * r) / vx if vx != 0 else 0        # Angolo di scivolamento posteriore
        ...
        # Aerodynamic drag and rolling resistance forces
        F_aero = 0.5 * self.rho * self.C_d * self.A * vx**2#
        F_roll = self.C_rr * self.mass * 9.81

        dx = vx                            # x˙ ≈ vx
        dy = vy                            # y˙ ≈ vy
        dtheta = r                         # θ˙ = r
        dvx = ax + r * vy - F_aero / self.mass - F_roll / self.mass                  # v˙x = ax + rvy
        dvy = (2 * Cf / m) * (delta - (vy + lf * r) / vx if vx != 0 else 0) +(2 * Cr / m) * (-(vy - lr * r) / vx if vx != 0 else 0) - r * vx  # v˙y
        dr = (2 * lf * Cf / Iz) * (delta - (vy + lf * r) / vx if vx != 0 else 0) - (2 * lr * Cr / Iz) * (-(vy - lr * r) / vx if vx != 0 else 0)      # r˙
        # Dynamics equations
        dx = np.array([
            vx,  # dx/dt
            dy,  # dy/dt
            dtheta,                                                      # dtheta/dt
            dvx,       # dvx/dt with resistive forces
            dvy,                  # dvy/dt
            dr                # dr/dt
        ])
